...
watchEffect(async () => {
  ...
  assetsBalanceUsd.value = (await getAssetUsdValue(assetsBalance.value, vault.value, 'off-chain')) ?? 0
  deltaUsd.value = (await getAssetUsdValue(delta.value, vault.value, 'off-chain')) ?? 0
})
